...
export function return_default_routes(atom_name:AtomName):Book.Definition.Api.Routes{
	return {
		find: {
			...
			call: async (route_request:book_types.RouteRequest) => {
				...
				const urn_bll = urn_core.bll.create(atom_name, route_request.token_object) as urn_core.bll.BLL<typeof atom_name>;
				const bll_res = await urn_bll.find(route_request.query.filter, route_request.query.options);
				return bll_res;
			}
		},
		find_id: {
			...
			call: async (route_request:book_types.RouteRequest) => {
				...
				const urn_bll = urn_core.bll.create(atom_name, route_request.token_object) as urn_core.bll.BLL<typeof atom_name>;
				const bll_res = await urn_bll.find_by_id(route_request.params.id, route_request.query.options);
				return bll_res;
			}
		},
		find_one: {
			...
			call: async (route_request:book_types.RouteRequest) => {
				...
				const urn_bll = urn_core.bll.create(atom_name, route_request.token_object) as urn_core.bll.BLL<typeof atom_name>;
				const bll_res = await urn_bll.find_one(route_request.query.filter, route_request.query.options);
				return bll_res;
			}
		},
		insert: {
		...
		update: {
			...
			call: async (route_request:book_types.RouteRequest) => {
				...
				const urn_bll = urn_core.bll.create(atom_name, route_request.token_object) as urn_core.bll.BLL<typeof atom_name>;
				const bll_res = await urn_bll.update_by_id(route_request.params.id, route_request.body);
				return bll_res;
			}
		},
		delete: {
