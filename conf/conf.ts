...
function _set_conf_from_env_variable(){
	...
	if(
		...
		api_config.encryption_rounds = Math.max(parseInt(process.env.URN_ENCRYPTION_ROUNDS),0);
	}
	if(
		...
		api_config.max_password_length = Math.max(parseInt(process.env.URN_MAX_PASSWORD_LENGTH),0);
	}
	if(
		typeof process.env.URN_STORAGE === 'string'
		&& process.env.URN_STORAGE !== ''
	){
		api_config.storage = process.env.URN_STORAGE as StorageType;
	}
	if(
		typeof process.env.URN_AWS_BUCKET_NAME === 'string'
		&& process.env.URN_AWS_BUCKET_NAME !== ''
	){
		api_config.aws_bucket_name = process.env.URN_AWS_BUCKET_NAME;
	}
	if(
		typeof process.env.URN_AWS_BUCKET_REGION === 'string'
		&& process.env.URN_AWS_BUCKET_REGION !== ''
	){
		api_config.aws_bucket_region = process.env.URN_AWS_BUCKET_REGION;
	}
	if(
		typeof process.env.URN_AWS_USER_ACCESS_KEY_ID === 'string'
		&& process.env.URN_AWS_USER_ACCESS_KEY_ID !== ''
	){
		api_config.aws_user_access_key_id = process.env.URN_AWS_USER_ACCESS_KEY_ID;
	}
	if(
		typeof process.env.URN_AWS_USER_SECRET_ACCESS_KEY === 'string'
		&& process.env.URN_AWS_USER_SECRET_ACCESS_KEY !== ''
	){
		api_config.aws_user_secret_access_key = process.env.URN_AWS_USER_SECRET_ACCESS_KEY;
	}
	if(
		...
		api_config.connect_on_init = !!process.env.URN_CONNECT_ON_INIT;
	}
}
...
function _check_max_password_length(){
	if(api_config.max_password_length >= 60){
		throw urn_exc.create_not_initialized(
...
function _check_if_storage_params_were_set(){
	switch(api_config.storage){
		case 'aws':{
			if(
				typeof api_config.aws_bucket_name !== 'string'
				|| api_config.aws_bucket_name === ''
			){
				throw urn_exc.create_not_initialized(
			...
			if(
				typeof api_config.aws_bucket_region !== 'string'
				|| api_config.aws_bucket_region === ''
			){
				throw urn_exc.create_not_initialized(
			...
			if(
				typeof api_config.aws_user_access_key_id !== 'string'
				|| api_config.aws_user_access_key_id === ''
			){
				throw urn_exc.create_not_initialized(
			...
			if(
				typeof api_config.aws_user_secret_access_key !== 'string'
				|| api_config.aws_user_secret_access_key === ''
			){
				throw urn_exc.create_not_initialized(
...
function _check_if_db_connections_were_set(){
	switch(api_config.db_type){
		case 'mongo':{
			if(api_config.mongo_main_connection === ''){
				throw urn_exc.create_not_initialized(
					`MISSING_MONGO_MAIN_CONNECTION`,
					`Missing mongo_main_connection in api_config.`
				);
			}
			if(api_config.mongo_trash_connection === ''){
				urn_log.warn(`You didn't set mongo_trash_connection.`);
			}
			if(api_config.mongo_log_connection === ''){
